...
export class MainMenu extends Scene {
  ...
  create() {
    ...
    const playButton = this.add
      .image(this.scale.width * 0.5, this.scale.height * 0.5 - (buttonHeight * 3) / 2, this.buttonPanel)
      .setDisplaySize(buttonWidth, buttonHeight);
    this.add.text(playButton.x, playButton.y, 'Play').setOrigin(0.5);

    const settingsButton = this.add
      .image(playButton.x, playButton.y + playButton.displayHeight + buttonOffset, this.buttonPanel)
      .setDisplaySize(buttonWidth, buttonHeight);
    this.add.text(settingsButton.x, settingsButton.y, 'Settings').setOrigin(0.5);

    const creditsButton = this.add
      .image(settingsButton.x, settingsButton.y + settingsButton.displayHeight + buttonOffset, this.buttonPanel)
      .setDisplaySize(buttonWidth, buttonHeight);
