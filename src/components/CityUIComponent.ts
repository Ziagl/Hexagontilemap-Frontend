...
export default class CityUIComponent implements IComponent {
  ...
  start(): void {
    ...
    scene.add
      .text(this.cityPosition.x, this.cityPosition.y, this.city.cityName, { font: this.font, fill: this.color })
      .setDepth(this.depth);
  }
  ...
  private createBorders() {
    ...
    if (this.city != undefined && this.city.cityBorders != undefined && this.city.cityBorders.length > 0) {
